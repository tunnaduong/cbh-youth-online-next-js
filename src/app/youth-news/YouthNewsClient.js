...
import { getYouthNews, votePost } from "@/app/Api";
import { useRouter } from "next/navigation";
import SkeletonPost from "@/components/home/skeletonPost";
import { useViewTracking } from "@/hooks/useViewTracking";

export default function YouthNewsClient() {
  ...
  const loadInitialPosts = useCallback(async () => {
    ...
    try {
      const response = await getYouthNews(1);
  ...
  const loadMorePosts = useCallback(async () => {
    ...
    try {
      const response = await getYouthNews(pagination.current_page + 1);
