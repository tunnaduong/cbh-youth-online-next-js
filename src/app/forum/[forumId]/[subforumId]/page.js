...
export default function Subforum({ params }) {
  ...
  useEffect(() => {
    const fetchSubforumData = async () => {
      try {
        ...
        // Fetch topics for this subforum
        const topicsResponse = await getSubforumPosts(subforumId);
    ...
    if (forumId && subforumId) {
      fetchSubforumData();
    }
  }, [forumId, subforumId, forumCategories]);
