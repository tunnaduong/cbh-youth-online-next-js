...
import { getPersonalizedFeedPosts, votePost } from "@/app/Api";
...
export default function FeedClient() {
  ...
  const loadInitialPosts = useCallback(async () => {
    ...
    try {
      const response = await getPersonalizedFeedPosts(1);
  ...
  const loadMorePosts = useCallback(async () => {
    ...
    try {
      const response = await getPersonalizedFeedPosts(pagination.current_page + 1);
