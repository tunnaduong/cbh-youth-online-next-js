...
import { getVotesForComment } from "@/app/Api";
...
export default function CommentVotesModal({ open, commentId, onClose }) {
  ...
  useEffect(() => {
    ...
    const fetchVotes = async () => {
      ...
      try {
        const response = await getVotesForComment(commentId);
        if (isActive) {
  ...
  const voteSummary = useMemo(() => {
    ...
    return { voteCount: votes.length, totalVotes };
  }, [votes]);
